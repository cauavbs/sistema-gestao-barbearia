...
O Firebase Admin SDK (usado no resto do backend) NÃO consegue validar
e-mail/senha diretamente — isso só é possível pelo SDK de cliente ou pela
API REST "Identity Toolkit" do Google. Por isso, aqui o backend faz uma
chamada HTTP para essa API REST para conferir a senha, e a partir daí
passa a controlar a sessão do usuário com o mecanismo de sessão do
próprio Flask (cookie assinado), sem depender mais do Firebase Auth no
navegador.

Mantive a mesma regra especial que existia no login.html original: se o
login falhar E o e-mail for o "cauadev@barbearia.com", cria essa conta
mestre automaticamente na primeira vez.
"""
...
def fazer_login():
    ...
    if codigo_erro in codigos_credencial_invalida and email == EMAIL_MESTRE_PADRAO:
        # Mesmo comportamento de "bootstrap" do login.html original:
        # cria essa conta mestre automaticamente na primeira vez que ela tentar logar.
        try:
