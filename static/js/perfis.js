document.addEventListener('DOMContentLoaded', async function () {
    // Autocomplete de barbeiros: usa a mesma rota do backend que a
    // página de Fichas usa pra gerenciar essa lista, então fica sempre
    // sincronizado.
    ...
    if (listaUsuariosEl) {
        listaUsuariosEl.addEventListener('click', async (e) => {
            if (e.target.classList.contains('btn-excluir')) {
                ...
                if (await mostrarConfirmacao("Deseja remover o acesso deste usuário do sistema?")) {
                    await fetch(`/api/perfis/${uid}`, { method: 'DELETE' });
                    await mostrarAlerta("Acesso removido com sucesso!");
    ...
    if (formEditar) {
        formEditar.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                await mostrarAlerta("Perfil atualizado com sucesso!");
                document.getElementById('modal-editar').style.display = 'none';
                await carregarUsuarios();
            } catch (erro) {
                await mostrarAlerta("Erro ao atualizar o perfil: " + erro.message);
            } finally {
    ...
    if (formPerfil) {
        formPerfil.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                await mostrarAlerta(`Perfil de ${nome} criado com sucesso!`);
                formPerfil.reset();
                await carregarUsuarios();
            } catch (erro) {
                await mostrarAlerta("Erro ao criar perfil: " + erro.message);
            } finally {
