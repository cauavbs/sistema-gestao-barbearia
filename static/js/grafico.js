    ...
    function renderizarGraficoComparativo(meses) {
        ...
        graficoComparativo = new Chart(canvas, {
            ...
            data: {
                labels: rotulos,
                datasets: [
                    { label: 'Entradas', data: meses.map(m => m.entradas), backgroundColor: '#16a34a' },
                    { label: 'Saídas', data: meses.map(m => m.saidas), backgroundColor: '#dc2626' },
                    { label: 'Lucro', data: meses.map(m => m.lucro), type: 'line', borderColor: '#1d4ed8', backgroundColor: '#1d4ed8', tension: 0.3 }
                ]
            },
            options: {
                responsive: true,
    ...
    function renderizarGraficoPagamentos(meses) {
        ...
        graficoPagamentos = new Chart(canvas, {
            ...
            data: {
                labels: rotulos,
                datasets: [
                    { label: 'Pix', data: meses.map(m => m.pix), backgroundColor: '#0891b2' },
                    { label: 'Débito', data: meses.map(m => m.debito), backgroundColor: '#2563eb' },
                    { label: 'Crédito', data: meses.map(m => m.credito), backgroundColor: '#7c3aed' },
                    { label: 'Dinheiro', data: meses.map(m => m.dinheiro), backgroundColor: '#16a34a' }
                ]
            },
            options: {
                responsive: true,
