document.addEventListener('DOMContentLoaded', async function () {
    ...
    async function salvarNoBanco(registro) {
        ...
        if (!resposta.ok) {
            ...
            alert(erro.erro || 'Erro ao lançar o gasto.');
            return;
        }
    ...
    if (fileInputInsumo) {
        fileInputInsumo.addEventListener('change', function (e) {
            ...
            if (file.size > 2 * 1024 * 1024) {
                alert("Para não sobrecarregar o banco de dados, escolha um comprovante de até 2MB.");
    ...
    if (form) {
        form.addEventListener('submit', function (event) {
            ...
            if (confirm("Deseja anexar um comprovante para este gasto?")) {
    ...
    if (listaGastos) {
        listaGastos.addEventListener('click', function (event) {
            if (event.target.classList.contains('btn-excluir')) {
                ...
                if (confirm("Deseja apagar este gasto do banco de dados?")) {
