    ...
    if (form) {
        form.addEventListener('submit', async function (event) {
            ...
            if (!resposta.ok) {
                ...
                await mostrarAlerta(erro.erro || 'Erro ao lançar a venda.');
                return;
            }

            await mostrarAlerta("Venda lançada com sucesso!");
            form.reset();
    ...
    if (listaVendas) {
        listaVendas.addEventListener('click', async function (event) {
            if (event.target.classList.contains('btn-excluir')) {
                ...
                if (await mostrarConfirmacao("Deseja apagar esta venda do freezer do banco de dados?")) {
