...
document.addEventListener('DOMContentLoaded', async function () {
    ...
    if (form) {
        form.addEventListener('submit', async function (event) {
            ...
            if (!resposta.ok) {
                ...
                await mostrarAlerta(erro.erro || 'Erro ao registrar fechamento.');
                return;
            }

            await mostrarAlerta("Fechamento lançado com sucesso!");
            form.reset();
    ...
    if (listaAtend) {
        listaAtend.addEventListener('click', async function (event) {
            ...
            if (event.target.classList.contains('btn-excluir') && perfilUsuario === 'master') {
                if (await mostrarConfirmacao("Deseja apagar este lançamento?")) {
    ...
    if (inputComprovante) {
        inputComprovante.addEventListener('change', async function () {
            if (this.files && this.files[0]) {
                const file = this.files[0];

                if (file.size > 2 * 1024 * 1024) {
                    await mostrarAlerta("O arquivo é muito grande. Escolha um comprovante de até 2MB.");
                ...
                reader.onload = async function (e) {
                    ...
                    if (modoPagamentoLote) {
                        ...
                        await mostrarAlerta(`${dados.quantidade_paga} lançamento(s) marcado(s) como pago(s) COM comprovante (e enviados aos Insumos)!`);
                        modoPagamentoLote = false;
                        idsLotePendente = [];
    ...
    if (btnAdicionarBarbeiro) {
        btnAdicionarBarbeiro.addEventListener('click', async () => {
            if (perfilUsuario !== 'master') {
                await mostrarAlerta("Apenas administradores podem adicionar barbeiros.");
                return;
            }
            const novoNome = await mostrarPrompt("Digite o nome do novo barbeiro (Ex: Lucas Queiroz):");
            if (novoNome && novoNome.trim() !== "") {
                ...
                if (!listaBarbeiros.includes(nomeFormatado)) {
                    ...
                    await mostrarAlerta(`Barbeiro "${nomeFormatado}" adicionado com sucesso!`);
                } else {
                    await mostrarAlerta("Este barbeiro já está na lista.");
                }
            }
        });
    }
    ...
    if (btnEditarBarbeiro) {
        btnEditarBarbeiro.addEventListener('click', async () => {
            if (perfilUsuario !== 'master') {
                await mostrarAlerta("Apenas administradores podem editar barbeiros.");
                return;
            }
            if (!barbeiroSelect.value) {
                await mostrarAlerta("Selecione um barbeiro na caixa ao lado para editar.");
                return;
            }
            const nomeAtual = barbeiroSelect.value;
            const novoNome = await mostrarPrompt("Edite o nome do barbeiro:", nomeAtual);

            if (novoNome && novoNome.trim() !== "" && novoNome.trim() !== nomeAtual) {
                const nomeFormatado = novoNome.trim();
                if (listaBarbeiros.includes(nomeFormatado)) {
                    await mostrarAlerta("Já existe um barbeiro com este nome.");
                    return;
                }
                ...
                if (index > -1) {
                    ...
                    renderizarOpcoesBarbeiros();
                    barbeiroSelect.value = nomeFormatado;
                    await mostrarAlerta(`Nome alterado de "${nomeAtual}" para "${nomeFormatado}".`);
                }
            }
        });
    }
    ...
    if (btnRemoverBarbeiro) {
        btnRemoverBarbeiro.addEventListener('click', async () => {
            if (perfilUsuario !== 'master') {
                await mostrarAlerta("Apenas administradores podem remover barbeiros.");
                return;
            }
            if (!barbeiroSelect.value) {
                await mostrarAlerta("Selecione um barbeiro na caixa ao lado para remover.");
                return;
            }
            const nomeAtual = barbeiroSelect.value;
            if (await mostrarConfirmacao(`Tem certeza que deseja remover "${nomeAtual}" da lista de opções?`)) {
                const index = listaBarbeiros.indexOf(nomeAtual);
                if (index > -1) {
                    listaBarbeiros.splice(index, 1);
                    await salvarListaBarbeiros();
                    renderizarOpcoesBarbeiros();
                    await mostrarAlerta(`Barbeiro "${nomeAtual}" foi removido da lista.`);
                }
            }
        });
    }
    ...
    if (btnPagarVisiveis) {
        btnPagarVisiveis.addEventListener('click', async function () {
            ...
            if (idsPendentes.length === 0) {
                await mostrarAlerta("Não há nenhum lançamento pendente visível na tela para pagar.");
                return;
            }

            if (await mostrarConfirmacao(`Encontrados ${idsPendentes.length} lançamento(s) pendente(s).\nDeseja anexar um ÚNICO comprovante para todos eles?`)) {
                modoPagamentoLote = true;
                idsLotePendente = idsPendentes;
                inputComprovante.click();
            } else {
                if (await mostrarConfirmacao("Deseja marcar todos como pagos SEM COMPROVANTE?")) {
                    const resposta = await fetch('/api/fichas/pagar-lote', {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
                    ...
                    const dados = await resposta.json();
                    await mostrarAlerta(`${dados.quantidade_paga} lançamento(s) marcado(s) como pago(s) SEM comprovante (e enviados aos Insumos)!`);
                    await carregarFichas();
                }
            }
        });
