"""
API de Gestão de Perfis (perfis.html).

Diferente do original — que precisava de uma SEGUNDA instância do
Firebase rodando no navegador só para criar contas sem deslogar o
admin, e pedia a senha antiga da pessoa pra poder trocar a senha dela
— aqui o backend usa o Admin SDK do Firebase, que tem permissão para
criar, editar e apagar a conta de qualquer usuário diretamente, sem
precisar da senha antiga e sem qualquer risco de interferir na sessão
de quem está logado fazendo a alteração.

Também corrige uma falha do original: lá, excluir um perfil só
apagava o registro no banco (o e-mail/senha continuavam válidos no
Firebase Auth, e se a pessoa tentasse logar de novo caía com perfil
"comum" — que por acidente tinha acesso igual ao master em algumas
páginas). Aqui, excluir um perfil também revoga o acesso de verdade,
apagando a conta no Firebase Auth.
"""
