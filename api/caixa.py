"""
API de Classificação e Troco: fechamento de caixa do dia e
configuração das taxas de máquina de cartão.

As taxas de débito/crédito ficam salvas no Firebase (nó
'configuracoesBarbearia/taxasMaquina'), então valem para qualquer
dispositivo que acessar o sistema.
"""
...
def salvar_taxas():
    ...
    if any(valor < 0 for valor in taxas.values()):
        return jsonify({"erro": "Nenhuma taxa pode ser negativa."}), 400
...
def criar():
    ...
    if any(valor < 0 for valor in (troco, pix, debito, debito2, credito, credito2)):
